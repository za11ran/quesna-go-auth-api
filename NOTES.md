...
**الباقي غير المنطقي:**
1. **الداش بورد** — `dashboard/` (Vite + React، RTL). اتعمل: الهيكل + الدخول (نموذج واحد لكل الأدوار) + Layout بالدور + شاشات الأدمن (نظرة عامة، طلبات التغيير بـ diff + موافقة/رفض، التجّار + إنشاء متجر). **الباقي**: شاشات dispatch (الطابور + تعيين + سوكيت لحظي)، vendor (منتجات/طلبات/عروض/بروفايل/رفع صور)، driver، وبقية شاشات الأدمن (أقسام/بانرات/دليفري/مشرفين/طلبات). تشغيل: `cd dashboard && npm install --ignore-scripts && npm run dev` (5173).
