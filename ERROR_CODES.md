...
- **التسجيل**: body = `name` + `phone` + `village_id`. اللغة تتبعت كـ **query**:
  `POST /api/auth/register?lang=ar` (اختياري، `ar`/`en`، الافتراضي `ar`).
  التطبيق بيبعت لغة الواجهة الحالية تلقائيًا — مش حقل المستخدم بيملاه.
- باقي البروفايل (الصورة، الإيميل، تاريخ الميلاد، النوع) بيتضاف بعد الدخول عبر
  `PATCH /api/auth/me`، ومنه حقل `lang` كمان لو المستخدم غيّر اللغة.
- بتُرجع دايمًا في `user.lang` — استخدمها لإرسال SMS/إشعارات بلغته لاحقًا.
- مفيش داعي تبعت اللغة في كل طلب — ترجمة رسائل الأخطاء مسؤولية التطبيق عبر `error_code`.
