...
- **التسجيل**: `name` + `phone` + `village_id` + `preferred_language` (اختياري، `ar`/`en`،
  الافتراضي `ar`). التطبيق بيبعت لغة الواجهة الحالية تلقائيًا — مش حقل المستخدم بيملاه.
- باقي البروفايل (الصورة، الإيميل، تاريخ الميلاد، النوع) بيتضاف بعد الدخول عبر
  `PATCH /api/auth/me`، ومنها `preferred_language` كمان لو المستخدم غيّر اللغة.
